...
def checkout(request):
    ...
    if not cart_items:
        logger.warning("order.cart_empty", extra={"user_id": user_id, "correlation_id": correlation_id})
        return Response({"error": "cart is empty"}, status=status.HTTP_400_BAD_REQUEST)

    try:
        wallet = Wallet.objects.get(user=user)
    except Wallet.DoesNotExist:
        logger.error("order.wallet_missing", extra={"user_id": user_id, "correlation_id": correlation_id})
        return Response({"error": "wallet not found"}, status=status.HTTP_404_NOT_FOUND)

    total = sum(item.product.price * item.quantity for item in cart_items)
    if wallet.balance < total:
        logger.info("order.insufficient_balance", extra={
            "user_id": user_id, "balance": float(wallet.balance),
            "total": float(total), "correlation_id": correlation_id,
        })
        return Response(
    ...
    with transaction.atomic():
        inventory_locks = list(
            Inventory.objects.select_for_update().filter(product_id__in=product_ids).order_by("id")
        )
        ...
        for ci in cart_items:
            ...
            if not inv or inv.quantity < ci.quantity:
                logger.info("order.stock_conflict", extra={
                    "product_id": str(ci.product_id), "product_name": ci.product.name,
        ...
        if wallet_lock.balance < total:
            logger.info("order.balance_conflict", extra={
                "user_id": user_id, "balance": float(wallet_lock.balance),
                "total": float(total), "correlation_id": correlation_id,
            })
            return Response(
